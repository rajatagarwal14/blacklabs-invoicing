  ...
  if (dbType === DatabaseType.postgre) {
    switch (sqliteType) {
      ...
      case 'INTEGER PRIMARY KEY AUTOINCREMENT':
        // BY DEFAULT, not ALWAYS. Several migrations follow SQLite's
        // rebuild-and-copy idiom (create table_new, INSERT ... SELECT including
        // "id", swap), and Postgres rejects an explicit insert into a GENERATED
        // ALWAYS identity column. BY DEFAULT still auto-generates when no id is
        // supplied. Sequences are resynced after migrations run — see
