...
> **This is not a terms of use document and has no legal effect.**
>
> It is a scaffold recording what the real terms must cover, to be drafted by
> counsel before BlackLabs Invoicing is sold to anyone. Nothing in it protects
> BlackLabs or binds a customer. Do not present it to a client as terms.

## Why there is no document here yet

Terms written for offline, single-user desktop software do not describe this
product and must not be adapted into it. A hosted service where the operator
holds the customer's database is a different arrangement in every respect that
matters legally, so the terms are being written for it rather than edited into
shape from something else.
...
liability that a shrink-wrapped tool would disclaim sits with BlackLabs.
...
- `LICENSE` — the licence this software is distributed under.
- `NOTICE` — third-party components and their required notices.
